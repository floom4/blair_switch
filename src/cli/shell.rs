          ...
          Ok(ref cmd) => {
            for available_cmd in commands::GENERAL_COMMANDS {
              if available_cmd.matches_pattern(&cmd) {
                available_cmd.run(intfs_view, fib, &mut mode, intfs_view.values().next().unwrap().clone(), &mut config, cmd);
                continue 'main;
              }
            }
        ...
        match input {
          Ok(ref cmd) => {
            for available_cmd in commands::INTF_COMMANDS {
              if available_cmd.matches_pattern(&cmd) {
                available_cmd.run(intfs_view, fib, &mut mode, intfs_view[&if_name[..]].clone(), &mut config, cmd);
                continue 'main;
              }
            }
            println!("Unknown command")
